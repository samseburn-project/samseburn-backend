...
public class ChallengeValidator {
    ...
    public static void validateCreate(String title,
        ...
        if (locationType == LocationType.오프라인 && Objects.equals(address, "") || locationType == LocationType.오프라인 && address == null) {
            throw new ApiRequestException("오프라인인 경우 주소를 입력하세요.");
        }

        if (locationType == LocationType.온라인 && address.trim().length() >= 1) {
